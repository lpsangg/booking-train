...
// Station mapping và route definitions - CHỈ 5 GA
...
// Route definitions cho từng tàu - CHỈ 5 GA
const trainRouteDefinitions = {
  // Tàu dài Hà Nội -> Hồ Chí Minh (10 routes mỗi tàu)
  'SE1': [
  ...
  // Tàu dài Hồ Chí Minh -> Hà Nội (10 routes mỗi tàu)
  'SE2': [
...
function calculateDistanceMultiplier(origin, destination) {
  const stations = ['Hà Nội', 'Vinh', 'Đà Nẵng', 'Nha Trang', 'Hồ Chí Minh'];
  ...
  const multipliers = { 1: 0.7, 2: 0.85, 3: 0.95, 4: 1.0 };
  ...
  for (const routeDefinition of definedRoutes) {
    ...
    for (const base of baseRoutes) {
      if ((base.origin === origin && base.destination === destination) ||
          (base.origin === 'Hà Nội' && base.destination === 'Vinh') ||
          (base.origin === 'Hà Nội' && base.destination === 'Đà Nẵng')) {
        baseRoute = base;
        break;
      }
    }
...
async function generateAllPricingFiles() {
  ...
  console.log('=== Starting generation với 5 ga: Hà Nội - Vinh - Đà Nẵng - Nha Trang - Hồ Chí Minh ===');

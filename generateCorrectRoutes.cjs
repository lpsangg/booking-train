...
// Route definitions CHÍNH XÁC theo yêu cầu
const trainRouteDefinitions = {
  // Tàu dài (10 routes mỗi tàu)
  'SE1': [
  ...
  // Tàu đoạn ngắn Hà Nội - Đà Nẵng (3 routes)
  'SE5': [
    { origin: 'Hà Nội', destination: 'Vinh' },
    { origin: 'Hà Nội', destination: 'Đà Nẵng' },
    { origin: 'Vinh', destination: 'Đà Nẵng' }
  ...
  // Tàu đoạn ngắn Hồ Chí Minh - Đà Nẵng (3 routes)
  'SE22': [
    { origin: 'Hồ Chí Minh', destination: 'Nha Trang' },
    { origin: 'Hồ Chí Minh', destination: 'Đà Nẵng' },
    { origin: 'Nha Trang', destination: 'Đà Nẵng' }
...
function calculateDistanceMultiplier(origin, destination) {
  ...
  const multipliers = { 1: 0.4, 2: 0.6, 3: 0.75, 4: 0.9, 5: 1.0 };
...
function generateRoutesForTrain(trainId, baseRoutes) {
  ...
  console.log(`  Generating ${definedRoutes.length} routes for ${trainId}`);
  ...
  for (const routeDefinition of definedRoutes) {
    ...
    // Nếu không tìm thấy exact match, dùng route đầu tiên làm base
    if (!baseRoute && baseRoutes.length > 0) {
      baseRoute = baseRoutes[0];
    }
    
    if (!baseRoute) {
      console.log(`    Skipping ${origin} -> ${destination} (no base route)`);
      continue;
    }
...
function processJsonFile(filePath, trainId) {
  try {
    ...
    console.log(`  Found ${baseRoutes.length} base routes in JSON`);
    
    // Generate routes theo definition
    const allRoutes = generateRoutesForTrain(trainId, baseRoutes);
    
    console.log(`  Final result: ${allRoutes.length} routes generated`);
...
async function generateCorrectPricingFiles() {
  ...
  console.log('=== CORRECT Route Generation Process ===');
  console.log('Expected routes:');
  console.log('- SE1, SE2, SE3, SE4, SE7, SE8, SE9, SE10: 10 routes each');
  console.log('- SE5, SE6: 3 routes each (Hà Nội - Đà Nẵng)');
  console.log('- SE22: 3 routes (Hồ Chí Minh - Đà Nẵng)');
  console.log('');
  ...
  console.log('\\n=== CORRECT Generation Complete ===');
}

generateCorrectPricingFiles().catch(console.error);
